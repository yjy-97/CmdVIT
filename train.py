...
class CenterLoss(nn.Module):
    ...
    def forward(self, points, centers):
        ...
        distances = torch.norm(points_expanded - centers_expanded, p=2, dim=3)  # (batch_size, num_points, num_centers)
        min_distances, _ = torch.min(distances, dim=2)
        center_loss = torch.mean(min_distances)
...
def valid(args, model, writer, test_loader, global_step):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        with torch.no_grad():
            ...
            eval_loss = loss_fct(logits, y) + 0.1 * cl_loss.to(args.device)
    ...
    report = metrics.classification_report(all_label, all_preds, target_names=[x.strip() for x in open('class.txt',
...
def train(args, model):
    ...
    while True:
        ...
        for step, batch in enumerate(epoch_iterator):
            batch = tuple(t.to(args.device) for t in batch)
            x, y = batch
            loss, atten = model(x, y)

            cl_loss = CenterLoss().forward(atten[0], atten[1]).to(args.device)
            loss = loss + 0.1 * cl_loss.to(args.device)
...
def main():
    ...
    parser.add_argument("--dataset", choices="", default="cifar10",
                        help="Which downstream task.")
    parser.add_argument("--model_type", choices="ViT-B_16",
                        default="ViT-B_16",
                        help="Which variant to use.")
    parser.add_argument("--pretrained_dir", type=str, default="/public/home/acw92jjscn/yjy/depvit/models/cmd.npz",
